...
def train(args, dataloader, im_backbone):
    ...
    def save_model(args, net, acc, epoch, location):
        ...
        if not os.path.isdir(location):
            os.mkdir(location)
        loc = os.path.join(location,
                           "ckpt_%s_%s_%s_%s.pth" % (args.model_name,
                                                     args.word_embed,
                                                     args.seed,
                                                     args.cl_replay_task_number))
...
def get_data_loader(args, split, batch_size=100, shuffle=True, num_workers=6, pin_memory=True):
    if split == "train":
        if args.cl_replay:
            dataset = dl.SMART_TrainCLData(args, split)
        else:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
                    prog='smart-vqa',
                    description='PathVQA using Smart101 algo')
    ...
    parser.add_argument(
        ...
        type=str,
        default="80:5:15",
        help="how to split train,val, test when both use the same instance list.",
    )
    parser.add_argument("--save_root", type=str, default="./data/", help="location to save intermediate files.")
    ...
    parser.add_argument('--check_ds', help='check dataset',  action='store_true') 

    args = parser.parse_args()

    if (args.check_ds):
        from datasets import load_dataset_builder
        ds_builder = load_dataset_builder("flaviagiammarino/vqa-rad")
    ...
    # vocab = build_vocab.process_text_for_puzzle(args)
    ...
    # train_loader = get_data_loader(args, "train", batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
